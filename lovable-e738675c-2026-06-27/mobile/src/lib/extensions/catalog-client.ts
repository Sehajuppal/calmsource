// Mirror of src/lib/extensions/catalog-client.ts — keep in sync.
...
export interface AddonVideo {
  ...
  thumbnail?: string; season?: number; episode?: number; overview?: string;
}

export interface AddonMeta extends AddonMetaPreview {
  cast?: string[]; director?: string[]; runtime?: string;
  videos?: AddonVideo[]; trailers?: { source: string; type: string }[];
}
...
export async function fetchCatalog(
  ext: InstalledExtension, catalog: ExtensionCatalog, signal?: AbortSignal,
...
export async function fetchMeta(
  baseUrl: string, type: string, id: string, signal?: AbortSignal,
