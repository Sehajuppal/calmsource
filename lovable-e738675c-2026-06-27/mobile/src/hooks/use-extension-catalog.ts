// Mirror of src/hooks/use-extension-catalog.ts — keep in sync.
...
export interface ExtensionCatalogRow {
  key: string; title: string; addonName: string; items: Title[];
}
...
function dedupeRow(items: Title[]): Title[] {
  const seen = new Set<string>(); const out: Title[] = [];
  for (const t of items) { if (seen.has(t.id)) continue; seen.add(t.id); out.push(t); }
  return out;
}

export function useExtensionCatalog(): ExtensionCatalogState {
  const [enabled, setEnabled] = useState<InstalledExtension[]>(() => extensionRepository.getEnabled());
  const [installedCount, setInstalledCount] = useState<number>(() => extensionRepository.getAll().length);
  ...
  const catalogsAvailable = catalogOwners.reduce((acc, e) => acc + e.manifest.catalogs.length, 0);

  useEffect(() => {
    if (catalogOwners.length === 0) {
      setRows([]); setStatus("empty"); setError(undefined);
      return;
    }
    ...
    (async () => {
      ...
      if (ok.length === 0) { setRows([]); setStatus("error"); setError("No catalogs responded."); }
      else { setRows(ok); setStatus("ready"); }
    })();
