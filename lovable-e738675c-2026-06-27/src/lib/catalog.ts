...
export type Title = {
  ...
  seasons?: number;
  episodes?: number;
  /** Optional intro markers (seconds) for timecode-aware Skip Intro */
  introStart?: number;
  introEnd?: number;
};
...
export const FILM_ROWS: { title: string; reason?: string; items: Title[] }[] = [
  { title: "New on CalmSource", reason: "Just added this week.", items: FILMS.slice(0, 6) },
  { title: "Critically acclaimed", reason: "Festival darlings and award winners.", items: FILMS.slice(3, 9) },
  { title: "Action & thrillers", reason: "High-stakes pulse-raisers.", items: FILMS.filter((f) => ["Action", "Thriller", "Crime"].some((g) => f.genres.includes(g))) },
  { title: "Drama & romance", reason: "Slow burns and quiet devastations.", items: FILMS.filter((f) => ["Drama", "Romance"].some((g) => f.genres.includes(g))) },
  { title: "Sci-fi & adventure", reason: "Worlds bigger than this one.", items: FILMS.filter((f) => ["Sci-Fi", "Adventure", "Documentary"].some((g) => f.genres.includes(g))) },
];
...
export const SERIES_ROWS: { title: string; reason?: string; items: Title[] }[] = [
  { title: "CalmSource Originals", reason: "Made here, only here.", items: SERIES.slice(0, 6) },
  { title: "Binge-worthy", reason: "Cleared-a-weekend kind of good.", items: SERIES.slice(3, 10) },
  { title: "Mystery & thriller", reason: "Sleep with the lights on.", items: SERIES.filter((s) => ["Mystery", "Thriller", "Horror", "Crime"].some((g) => s.genres.includes(g))) },
  { title: "Sci-fi universe", reason: "Stories that bend the rules.", items: SERIES.filter((s) => s.genres.includes("Sci-Fi")) },
  { title: "Drama spotlight", reason: "Performances worth talking about.", items: SERIES.filter((s) => ["Drama", "Action"].some((g) => s.genres.includes(g))) },
];
