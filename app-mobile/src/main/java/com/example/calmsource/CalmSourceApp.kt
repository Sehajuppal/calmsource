...
class CalmSourceApp : Application() {
     ...
     * so a secret saved right after a cold start is never written to a throwaway in-memory store
     * and then lost when the encrypted store is installed (bug #7). The "ext_" account namespace
     * keeps these separate from debrid OAuth tokens (bug #31), and it reuses the same encrypted
     * file/key scheme so previously-saved extension secrets remain readable.
     */
     ...
     * switching discovery/enrichment into low-memory mode. Previously memory pressure was only
     * handled indirectly during playback; this wires the OS-level signal.
     */
